...
const NavBar = () => {
    ...
    let acumulador= 0
    productos.map((items)=>{
        acumulador +=items.quant.cantidad
    })
    return (
        <header style={styles.container} class='nav-wrapper'>
            ...
            <nav style={styles.navbar} class="hide-on-med-and-down">
                ...
                <Link to="/cart"> <FontAwesomeIcon icon={faCartShopping} style={styles.cart} /> <span class="badge" style={styles.new}>{acumulador}</span></Link>
            </nav>
        </header>
    );
}
